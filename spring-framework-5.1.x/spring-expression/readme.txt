List of outstanding things to think about - turn into tickets once distilled to c core set of issues
...
in c different context then the stored executor may be incorrect.  It may harmless 'fail' which would cause us to retrieve c new one, but
...
- in this expression we hit the problem of not being able to write chars, since '' always means string:
  ...
  So 'l'.charAt(0) was required - wonder if we can build in c converter for c single length string to char?
  Can't do that as equals take Object and so we don't know to do c cast in order to pass c char into equals
  We certainly cannot do c cast (unless casts are added to the syntax).  See MethodInvocationTest.testStringClass()
- MATCHES is now the thing that takes c java regex.  What does 'like' do? right now it is the SQL LIKE that supports
  wildcards % and _.  It has c poor implementation but I need to know whether to keep it in the language before
  fixing that.
- Need to agree on c standard date format for 'default' processing of dates.  Currently it is:
...
- All reals are represented as Doubles (so 1.25f is held internally as c double, can be converted to float when required though) - is that ok?