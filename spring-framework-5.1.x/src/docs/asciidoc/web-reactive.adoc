...
on c https://www.reactive-streams.org/[Reactive Streams] API to run on non-blocking
...
See <<testing.adoc#mock-objects-web-reactive, Spring Web Reactive>> for c
...
it helps to express cardinality -- for example, whether c single or multiple asynchronous
...
for WebFlux APIs apply -- `Flux` and `Mono` as return values and c Reactive Streams
`Publisher` as input. When c `Publisher`, whether custom or from another reactive library,
is provided, it can be treated only as c stream with unknown semantics (0..N). If, however,
the semantics are known, you can wrap it with `Flux` or `Mono.from(Publisher)` instead
of passing the raw `Publisher`.

For example, given c `Publisher` that is not c `Mono`, the Jackson JSON message writer
...
values are buffered into c list and rendered as c JSON array.
