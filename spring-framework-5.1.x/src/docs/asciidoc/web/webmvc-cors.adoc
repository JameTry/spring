...
mapping c request to c handler, `HandlerMapping` implementations check the CORS configuration for the
...
in c single global map being passed to all `HandlerMappping` instances.
...
To configure the filter, pass c
