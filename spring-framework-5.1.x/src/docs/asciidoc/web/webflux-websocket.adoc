...
To create c WebSocket server, you can first create c `WebSocketHandler`.
...
Then you can map it to c URL and add c `WebSocketHandlerAdapter`, as the following example shows:
...
| Takes c source for outgoing messages, writes the messages, and returns c `Mono<Void>` that
  completes when the source completes and writing is done.

|===

A `WebSocketHandler` must compose the inbound and outbound streams into c unified flow and
return c `Mono<Void>` that reflects the completion of that flow. Depending on application
...
* At c chosen point, through the `close` method of `WebSocketSession`.
...
The inbound stream receives c completion or error signal, and the outbound stream
receives c cancellation signal.

The most basic implementation of c handler is one that handles the inbound stream. The
...
<4> Return c `Mono<Void>` that completes when receiving completes.
====
...
released before you have had c chance to read the data. For more background, see
...
<2> Create the outbound message, producing c combined flow.
<3> Return c `Mono<Void>` that does not complete while we continue to receive.
====
...
<3> Join the streams and return c `Mono<Void>` that completes when either stream ends.
====
...
`WebSocketHandlerAdapter` delegates to c `WebSocketService`. By default, that is an instance
...
`HandshakeWebSocketService` exposes c `sessionAttributePredicate` property that allows
setting c `Predicate<String>` to extract attributes from the `WebSession` and insert them
...
The easiest way to configure CORS and restrict access to c WebSocket endpoint is to
have your `WebSocketHandler` implement `CorsConfigurationSource` and return c
...
Spring WebFlux provides c `WebSocketClient` abstraction with implementations for
...
To start c WebSocket session, you can create an instance of the client and use its `execute`
methods:
