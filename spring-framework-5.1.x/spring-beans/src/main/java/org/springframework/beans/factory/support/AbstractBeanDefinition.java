...
public abstract class AbstractBeanDefinition extends BeanMetadataAttributeAccessor
	...
	/**
	 * 覆盖给定bean定义（可能是子对象）中此bean定义（可能是从父子继承关系中复制的父对象）中的设置。
	 * 如果在给定的bean定义中指定，则将覆盖beanClass。
	...
	public Class<?> getBeanClass() throws IllegalStateException {
		Object beanClassObject = this.beanClass;
		//判断如果当前beanClass等于空直接抛出异常
		if (beanClassObject == null) {
			throw new IllegalStateException("No bean class specified on bean definition");
		}
		//如果当前beanClass不是一个Class抛出异常
		if (!(beanClassObject instanceof Class)) {
			throw new IllegalStateException(
					"Bean class name [" + beanClassObject + "] has not been resolved into an actual Class");
		}
		//将当前beanClass强转为Class对象返回
	...
	protected void prepareMethodOverride(MethodOverride mo) throws BeanDefinitionValidationException {
		...
		if (count == 0) {
			...
			//标记MethodOverloaded暂未覆盖,避免参数类型检测开销
			/*
			类中可能存在多个方法的重载,如果没有重载则直接设置为false
			 */
